...
class FlippingCounter:
    ...
    def iter_alignments_flipped(self, samfile: str, unique: bool=True, yield_line: bool=False) -> Iterable:
        """Iterates over the bam/sam file and yield Read objects
        ...
        for i, read in enumerate(fin):
            ...
            if bc not in self.valid_bcset:
                if self.filter_mode:
                    ...
                    self.valid_bcset.add(bc)
            # NOTE: this is where I do the swapping in the flipping control mode !!!!
            strand = '+' if read.is_reverse else '-'
            chrom = fin.get_reference_name(read.rname)  # this is return a string otherwise read.rname for the integer
            if chrom.startswith('chr'):
                # I have to deal with incongruences with the .gft (what is cellranger doing???)
    ...
    def count(self, samfile: str, multimap: bool, cell_batch_size: int=100, molecules_report: bool=False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[str]]:
        ...
        for r in self.iter_alignments_flipped(samfile, unique=not multimap):
...
class PolyTCounter:
    ...
    def iter_alignments_filterT(self, samfile: str, unique: bool=True, yield_line: bool=False) -> Iterable:
        """Iterates over the bam/sam file and yield Read objects
        ...
        for i, read in enumerate(fin):
            ...
            # This filter away the non polyT
            if (clip5 >= vcy.MIN_POLYT and read.is_reverse):
                if not read.query_sequence[:clip5].count('T') > clip5 * 0.9:
                    continue
            elif (clip3 >= vcy.MIN_POLYT and not read.is_reverse):
                if not read.query_sequence[-clip3:].count("T") > clip3 * 0.9:
                    continue
            else:
                continue
    ...
    def count(self, samfile: str, multimap: bool, cell_batch_size: int=100, molecules_report: bool=False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[str]]:
        ...
        nth = 0
        # Loop through the aligment of the samfile
        for r in self.iter_alignments_filterT(samfile, unique=not multimap):
            if len(self.cell_batch) == cell_batch_size and r.bc not in self.cell_batch:
                # Perfrom the molecule counting
                nth += 1
                logging.debug(f"Counting for batch {nth}, containing {len(self.cell_batch)} cells and {len(self.reads_to_count)} reads")
