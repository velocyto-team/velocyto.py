...
You might want to mask expressed repetitive elements, since those count could costitute a confounding factor in the downstream analysis.
Dowloaded an appropriate expressed repeat mask annotation (for example `from UCSC genome browser <https://genome.ucsc.edu/cgi-bin/hgTables?hgsid=611454127_NtvlaW6xBSIRYJEBI0iRDEWisITa&clade=mammal&org=Mouse&db=mm10&hgta_group=allTracks&hgta_track=rmsk&hgta_table=0&hgta_regionType=genome&position=chr12%3A56694976-56714605&hgta_outputType=primaryTable&hgta_outputType=gff&hgta_outFileName=mm10_rmsk.gtf>`_ and **make sure to select GTF**  as output format).
And run the following command to prepare the file to the next step:
...
The ``run`` default counting logic is compatible with both 10X Genomics v1/v2 and InDrops 3' chemistry.
However, for data generated by 10X Genomics platform using the ``cellranger`` pipeline, we provide to use the shortcut ``run10x`` (see below).
...
.. note::
    Execution time is ~3hm for a typical sample but might vary significantly by sequencing depth and cpu power. 
    
.. warning::
   Running velocyto without specifying a filtered barcode set (:code:`-b/--bcfile` option) is not recommended, do it at your own risk. It will use all the cell barcode it encounters. Doing so might result in long runtimes and huge memory alloactions.
...
The rules used to call spliced, unspliced and ambiguous molecules from the reads mapping evidences can be set using the :code:`--logic` parameter .
modified using one of the different logics supported.
...
All the above were designed and validated for 10X Genomics 3' chemistry in mind but they are generalizable to similar chemistries.

.. hint::
    Custom logics supporting peculiarities of ohter chemistries can be implemented simply by creating a class that inherits from :ref:`Logic <logicapi>`.
